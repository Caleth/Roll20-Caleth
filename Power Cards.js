...
var PowerCards_Version = "2.4.11";
var PowerCards_LastUpdated = "March 28th, 2015 ~ 2:50 am eastern";
...
PowerCard.Process = function(msg, player_obj) {
	...
	var Title = "<div style='margin-left: -10px; box-shadow: " + PowerCard.boxshadow + "; border-radius: " + PowerCard.corners + "px;'><div style='" + TitleStyle + "' class='showtip tipsy' title='" + PowerCard.title + "'>" + PowerCard.name;
	...
	if (msg.inlinerolls !== undefined) {
		...
		var RollExpression = "";
		var RollValue = 0;
		_.each(msg.inlinerolls, function(roll, i){
			RollExpression = roll.expression;
			RollValue = buildInline(roll);
	...
	var urls = [],
		...
		formatter = function(s) {
			return s
				...
				.replace(/\]\_\[/g, "<hr style='border: 0; height: 0; border-top: 1px solid rgba(0, 0, 0, 0.1); border-bottom: 1px solid rgba(255, 255, 255, 0.3); margin-bottom: 3px; margin-top: 3px;'/>")
...
function buildInline(inlineroll) {
	...
	if (expandedCheck) {
		rollOut = '<span style="' + InlineRollStyle + '" title="Roll: ' + inlineroll.expression + '<br>Results: ' + values.join("") + ' = ' + inlineroll.results.total;
		rollOut += '" class="a inlinerollresult showtip tipsy';
	...
	rollOut += _.map(inlineroll.results.tableentries, function(l) {
		return '<span style="' + InlineRollStyle + '" title="Table: ' + l.table + ' ' + 'Weight: ' + l.weight + '" class="inlinerollresult showtip tipsy-n">' + l.name + '</span>';
	}).join('');
	return rollOut;
}
