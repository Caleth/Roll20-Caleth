...
// USER CONFIGUATION
    var POWERCARD_DEFAULT_FORMAT = "dnd4e";
...
PowerCardScript.Process = function(msg){
    ...
    if (PowerCard.charid !== undefined && PowerCard.emote !== undefined && POWERCARD_CUSTOM_EMOTE) {
        ...
        if (POWERCARD_SHOW_AVATAR) {
            PowerCard.emote = "<div style='display: table-cell; width: 50px;'><img src='" + Character.get("avatar") + "' style='height: 50px; width: 50px;'></img></div><div style='display:table-cell; width: calc(100%-50px); vertical-align: middle; font-style: italic; text-align: center;'>" + PowerCard.emote + "</div>";
        } else {
            PowerCard.emote = "<div style='display:block; width: 100%; vertical-align: middle; font-style: italic; text-align: center;'>" + PowerCard.emote + "</div>";
        }
    }
    ...
    // CREATE TITLE STYLE
    var TitleStyle = " font-family: Georgia; font-size: medium; font-weight: normal; letter-spacing: 0.25px; text-shadow: 1px 1px #000; text-align: center; vertical-align: middle; padding: 2px 0px; margin: 0px 0px 0px -10px; border: " + POWERCARD_BORDER_SIZE + "px solid " + POWERCARD_BORDER_COLOR + ";";
    ...
    if (!POWERCARD_USE_PLAYER_COLOR) {
        if (PowerCard.usage !== undefined && PowerCard.txcolor === undefined && PowerCard.bgcolor === undefined) {
            // PRESET TITLE COLORS
            if (PowerCard.usage.toLowerCase().indexOf("encounter") != -1) {
                TitleStyle += Encounter;
            }
            else if (PowerCard.usage.toLowerCase().indexOf("daily") != -1) {
                TitleStyle += Daily;
            }
            else if (PowerCard.usage.toLowerCase().indexOf("item") != -1) {
                TitleStyle += Item;
            }
            else if (PowerCard.usage.toLowerCase().indexOf("recharge") != -1) {
                TitleStyle += Recharge; 
                PowerCard.usage = PowerCard.usage.replace(/\d/g, function (n)
                {
                    return "</span><span style='font-family: dicefontd6;'>" + String.fromCharCode(96 + parseInt(n)) + "</span><span style='font-family: Tahoma; font-size: small; font-weight: normal;'>"
                });
            }
    ...
    var IgnoredTags = "format, emote, name, usage, action, defense, txcolor, bgcolor, leftsub, rightsub, ddn, desc, crit, title, whisper, orowbg, orowtx, erowbg, erowtx, charid";
        ...
        if (ReservedTags.indexOf(Tag) != -1) {
            ...
            if (Tag.toLowerCase() == "damage") {
                for (var DamageCount = 0; DamageCount < NumberOfDmgRolls; DamageCount++) {
                    ...
                    else DisplayCard += "<div style='" + RowStyle + RowBackground + "'>" + (POWERCARD_SHOW_ATTACK_DAMAGE_TAGS ? "<b>Hit:</b> " : "") + "$[d" + DamageCount + "]</div>";
                }
            }
